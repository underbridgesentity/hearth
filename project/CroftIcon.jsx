...
function Icon({ name, color = '#3B5BFF', size = 44, radius = 14, glyph = 22, bare = false }) {
  size = Number(size) || 44;
  radius = Number(radius) || 14;
  glyph = Number(glyph) || 22;
  bare = bare === true || bare === 'true';
  const draw = GLYPHS[name] || GLYPHS.goal;
  const svg = (
    <svg width={glyph} height={glyph} viewBox="0 0 24 24" fill="none">{draw(color)}</svg>
  );
  if (bare) return svg;
  return (
    <div style={{
